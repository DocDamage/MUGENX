...
local specs = {
    ...
    {id = "combo_trainer", status = "active", init = true, group = "training"},
    {id = "trials", status = "active", init = true, group = "training"},
    ...
    {id = "achievements", status = "experimental", init = true, group = "progression"},
    ...
    {id = "rpg_quests", status = "experimental", init = true, group = "rpg", depends = {"rpg_core"}},
...
function registry.list(status)
    ...
    for _, spec in ipairs(registry.specs) do
        if status == nil or spec.status == status then
            out[#out + 1] = spec
        end
    end
    return out
...
function registry.counts()
    ...
    for _, spec in ipairs(registry.specs) do
        ret[spec.status] = ret[spec.status] + 1
    end
    return ret
end
